...
def plan_route_best(start, end, obstacles, flight_altitude, safety_radius):
    ...
    for i in range(n):
        for j in range(i+1, n):
            if is_path_safe(unique[i], unique[j], high_obstacles, flight_altitude):
...
def plan_route_left(start, end, obstacles, flight_altitude, safety_radius):
    ...
    for _ in range(20):
        ...
        min_lat, max_lat, min_lng, max_lng = bounds
        center_lat = (min_lat + max_lat) / 2
        offset_meters = safety_radius * 4.0   # 足够大，确保绕开
        offset_lng = offset_meters / (111320 * math.cos(math.radians(center_lat)))
        left_point = (center_lat, min_lng - offset_lng)
        
        # 二次安全验证：如果绕行点仍在障碍物内，再向左偏移一倍
        safe = True
        for obs in remaining:
...
def plan_route_right(start, end, obstacles, flight_altitude, safety_radius):
    ...
    for _ in range(20):
        ...
        min_lat, max_lat, min_lng, max_lng = bounds
        center_lat = (min_lat + max_lat) / 2
        offset_meters = safety_radius * 4.0
        offset_lng = offset_meters / (111320 * math.cos(math.radians(center_lat)))
...
def plan_route():
    ...
    alt = st.session_state.flight_altitude
    safe = st.session_state.safety_radius
    
    if mode == "best":
        route = plan_route_best(start, end, st.session_state.obstacles, alt, safe)
    elif mode == "left":
        route = plan_route_left(start, end, st.session_state.obstacles, alt, safe)
    else:
        route = plan_route_right(start, end, st.session_state.obstacles, alt, safe)
...
def get_eta():
    return get_remaining() / st.session_state.flight_speed if st.session_state.flight_speed > 0 else 0
...
with tab1:
    ...
    with col_btn1:
        if st.button("🎯 规划航线", key="plan", use_container_width=True, type="primary"):
            plan_route()
            save_data()
            st.success("航线规划完成！")
    with col_btn2:
        if st.button("💾 保存数据", key="save", use_container_width=True):
            save_data()
            st.success("已保存")
    with col_btn3:
        if st.button("🗑️ 清空障碍物", key="clear", use_container_width=True):
            st.session_state.obstacles = []
            st.session_state.current_route = []
            save_data()
            st.success("已清空")
    with col_btn4:
        if st.button("🗺️ 重置视图", key="reset_view", use_container_width=True):
            st.session_state.map_center = [32.2341, 118.7494]
            st.session_state.start_point = (32.2345, 118.7492)
            st.session_state.end_point = (32.2337, 118.7496)
            st.session_state.obstacles = []
            ...
            st.success("已重置")
    with col_btn5:
        if st.button("❌ 取消模式", key="cancel_mode", use_container_width=True):
            st.session_state.set_mode = None
...
with tab2:
    ...
    with c1:
        if not st.session_state.mission_active:
            if st.button("▶️ 开始任务", key="start", use_container_width=True, type="primary"):
                start_mission()
                st.rerun()
        else:
            st.button("✅ 自主飞行中", use_container_width=True, disabled=True)
    with c2:
        if st.session_state.mission_active and not st.session_state.mission_paused:
            ...
            if st.button("▶️ 恢复", key="resume", use_container_width=True):
                resume_mission()
                st.rerun()
        else:
            st.button("⏸️ 暂停", use_container_width=True, disabled=True)
    with c3:
        if st.button("⏹️ 停止", key="stop", use_container_width=True):
            stop_mission()
            st.rerun()
    with c4:
    ...
    with s4:
        st.metric("剩余距离", f"{get_remaining():.0f} m")
    with s5:
...
with tab3:
    ...
    with col2:
        ...
        data = {
            "参数": ["当前位置", "当前航点", "总航点数", "飞行速度", "飞行高度", "安全半径", "绕行模式", "电池电量", "已用时间", "剩余距离", "预计到达"],
            "数值": [
                pos,
        ...
        if st.session_state.heartbeat_history:
            st.dataframe(pd.DataFrame(st.session_state.heartbeat_history[-10:]), use_container_width=True)
